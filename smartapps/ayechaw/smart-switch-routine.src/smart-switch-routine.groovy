...
def OptionalSetup() {
	...
	section ("Send to...") {
        ...
        section ("Link Presence detection to the state of a Switch...") {
        	input "presenceSwitch", "capability.switch", title: "Presence Switch", multiple: false, required: false
            }
        section {
        	href(name: "toMainSetupSetup", title: "Back to Main Setup Page", page: "MainSetup")
   		}
...
def initialize() {
    ...
    if (presenceSwitch) {
    		subscribe(presenceSwitch, "switch", pswitch)
        	}
        else {
...
//States what to do if there is a Presence event change - using optional Presence Switch
def pswitch(evt) {
	log.debug "evt.name: $evt.value"
    if (evt.value == "off") {
    	log.debug "Presence Switch - everyone is away"
        	if (switchIsOn()) {
        		log.debug "starting Switch On - Away sequence"
            	takeActionOnAway()
            }
       		...
       		log.debug "Presence Switch - someone is home"
     		if (switchIsOn()) {
       			log.debug "starting Switch On - Home sequence"
           		takeActionOnHome()
            	}
...
private everyoneIsAway() {
	def result = true
    	if (presenceSwitch) {
        	if (presenceSwitch.currentSwitch == "on") {
        		result = false
       			}
            }
    	else {
...
// ControlSwitch: returns true if switch is on,
// false otherwise.
private switchIsOn() {
	def result = true
